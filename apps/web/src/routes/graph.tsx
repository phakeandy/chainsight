    ...
    return (
        <div className="relative h-screen w-full overflow-hidden bg-background">
            <div className="fixed left-0 right-0 top-0 z-20 flex h-14 items-center justify-between border-b bg-background px-4">
                <div className="flex items-center gap-4">
                    ...
                    <Badge variant="secondary" className="text-xs">
                        {graphData.nodes.length} 条记录 / {graphData.links.length} 条关联
                    </Badge>
                </div>
            ...
            <div
                ...
                className={`absolute top-14 transition-all duration-200 ${selectedNode ? 'left-32 right-32' : 'left-32 right-0'}`}
                ...
                {isLoading ? (
                    ...
                    <ForceGraph2D
                        ...
                        linkLabel={(link) => {
                            ...
                            return l.edgeType === 'semantic' ? `语义关联 ${Math.round(l.score * 100)}%` : `直接传播 ${Math.round(l.score * 100)}%`
                        }}
